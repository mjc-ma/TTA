...
class DDIMSampler(object):
    ...
    def cond(self,score_corrector, x,pred_x0,t,scale=1.):
        grediant = score_corrector(pred_x0, t)
        x = x + grediant * scale
        return x
    ...
    def p_sample_ddim(self, x, c, t, index, negative_conditioning=None,
                      ...
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      injected_features=None, negative_prompt_alpha=1.0,scale=1.0
                      ):
    ...
    # print('t',t)
        if score_corrector is not None and t < 300:
            with torch.enable_grad():
                x = x.detach().requires_grad_()
                pred_x0 = (x - sqrt_one_minus_at * e_t) / a_t.sqrt()
                x = self.cond(score_corrector, x, pred_x0, t ,scale=scale)
        pred_x0 = (x - sqrt_one_minus_at * e_t) / a_t.sqrt()
        # e_t = self.cond(score_corrector, x, pred_x0, t ,scale=scale)
