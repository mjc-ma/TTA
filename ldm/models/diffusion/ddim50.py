...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True, strength = 1.):
        self.ddim_timesteps = make_ddim_timesteps(ddim_discr_method=ddim_discretize, num_ddim_timesteps=ddim_num_steps,
                                                    num_ddpm_timesteps=ddim_num_steps,verbose=verbose, strength=strength)
        # alphas_cumprod = self.model.alphas_cumprod[-self.ddim_timesteps.shape[0]:]
        alphas_cumprod = self.model.alphas_cumprod[:self.ddim_timesteps.shape[0]]

        assert alphas_cumprod.shape[0] == ddim_num_steps, 'alphas have to be defined for each timestep'
        to_torch = lambda x: x.clone().detach().to(torch.float32).to(self.model.device)

        self.register_buffer('betas', to_torch(self.model.betas))
        self.register_buffer('alphas_cumprod', to_torch(alphas_cumprod))
        self.register_buffer('alphas_cumprod_prev', to_torch(self.model.alphas_cumprod_prev[-self.ddim_timesteps.shape[0]:]))
    ...
    def calculate_weighted_difference(self,tensor1, tensor2):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
        ...
        for i, step in enumerate(iterator):
            ...
            img, pred_x0 = outs
            ## 新建一个0-50的llist
            call_back_ddim_timesteps_list = np.flip(np.arange(0, 50))
