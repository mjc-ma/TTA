...
##### save feature_map in local
    ...
    def ddim_sampler_callback_(pred_x0, xt, i):
        ...
        print(predicted_samples_path)
        print(predicted_samples_path_)
    ...
    ## For each class, sub-sample 'num' images
        ...
        for subdir, _, files in sorted(os.walk(_dir)):
            ...
            for file in sorted(files):
                ...
                start_code = model.q_sample(init_latent, t, noise=torch.randn_like(init_latent))
                ...
                with precision_scope("cuda"):
                    with model.ema_scope():
                        ...
                        c = model.get_learned_conditioning(prompts)
                        shape = [opt.C, opt.H // opt.f, opt.W // opt.f]
                        samples_ddim, _ = model.sample(S=ddim_steps,
                                                        conditioning=c,
