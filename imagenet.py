...
def get_prefix_samples_(root, folder_to_idx, extensions, shuffle=False):
...
def get_prefix_samples(root,num, folder_to_idx, extensions, shuffle=False):
    """Make dataset by walking all images under a root.
    ...
    for folder_name in sorted(os.listdir(root)):
        ...
        for _, _, fns in sorted(os.walk(_dir)):
            ...
            for fn in sorted(fns):
                ...
                if has_file_allowed_extension(fn, extensions):
                    # print(has_file_allowed_extension(fn, extensions))
                    count += 1
                    path = os.path.join(folder_name, fn)
                    item = (root, path, folder_to_idx[folder_name])
                    # print(item)
    ...
    def load_annotations(self):
        ...
        for l in load_list:
            ...
            if self.ann_file is None:
                folder_to_idx = find_folders_(data_prefix)
                sample = get_prefix_samples_(
                    data_prefix,
                    folder_to_idx,
